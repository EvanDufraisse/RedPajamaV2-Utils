...
class Worker:
    ...
    def __init__(
        ...
        self._logger.info(f"__MINHASH_PERM_CHECKSUM__ " f"{self._minhash.checksum}")
        ...
        # build input paths
        self._input_uri_list = list(
            map(lambda x: os.path.join(self._input_base_uri, x), input_listings)
        )
        ...
        self._reader = Reader(
            input_spec=InputSpec, threads=1, s3_client=client, logger=self._logger
        )
    ...
    def process_record(self, idx: int, record, uri_id: str, snapshot_id: str):
        return self.__process_record(idx, record, uri_id, snapshot_id)

    def __process_record(self, idx: int, record, uri_id: str, snapshot_id: str):
        ...
        # ccnet_quality_signals = {
        #     "ccnet_length": ((0, len(document), float(record.length)),),
        #     "ccnet_original_length": (
        #         (0, len(document), float(record.original_length)),
        #     ),
        #     "ccnet_nlines": ((0, len(document), float(record.nlines)),),
        #     "ccnet_original_nlines": (
        #         (0, len(document), float(record.original_nlines)),
        #     ),
        #     "ccnet_language_score": ((0, len(document), float(record.language_score)),),
        #     "ccnet_perplexity": ((0, len(document), float(record.perplexity)),),
        #     "ccnet_bucket": ((0, len(document), _ccnet_bucket_to_int(record.bucket)),),
        # }
    ...
    def __process_uri(self, docs_to_fetch: int, uri: str):
        ...
        for idx, record in self._reader.read(
            ...
            if num_docs % self._flush_interval == 0:
                ...
                signal_writer.flush()
                self._monitor_queue.put({"lang": self._lang, "num_docs": docs_added})
                docs_added = 0
    ...
    def run(self):
        ...
        for i, uri in enumerate(self._input_uri_list, start=1):
            ...
            if docs_to_fetch <= 0 < self._max_docs:
                self._logger.info(f"Reached max docs {self._max_docs} at {uri}")
                break
            ...
            try:
                ...
                with open(self._failed_input_file, "a+") as f:
                    f.write(f"{uri}\n")
                self._logger.error(
                    f"__URI_FAIL__ {uri} with exception: "
        ...
        self._logger.info(
            f"Worker {self._job_id} Completed. " f"Processed {total_docs} documents."
        )
