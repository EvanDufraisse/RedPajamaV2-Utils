...
from redpajama.dedupe.utils import optimal_param, generate_signature
...
class MinHash:
    ...
    def __init_minhash(self, num_permutations):
        ...
        self._permutations = np.array(
            [
                (
                    self._rng.randint(1, self._mersenne_prime, dtype=np.uint64),
                    self._rng.randint(0, self._mersenne_prime, dtype=np.uint64),
                )
        ...
        self._checksum = hashlib.sha256(self._permutations.tobytes()).hexdigest()
    ...
    def compute_banded_signatures(
        ...
        signatures = {
            sig_key: [
                bytes(minhashes[start:end].byteswap().data) for start, end in hashrange
            ]
